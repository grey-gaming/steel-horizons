...
- [x] **P1-13 — Persistence and replay skeleton.**
...
- [x] **P1-14 — Authenticated REST walking skeleton and generated client.**
...
Focused proof: `cargo test --locked` — 315 tests including 12 command sequencing/idempotency tests (same-id same-envelope idempotency, same-id different-envelope conflict, same-id different-command-body conflict, expected_tick mismatch, running queues replayable, running applies control immediately, paused applies immediately, queued command applied at next tick, sorted by server_sequence, session receipts survive new-game, expected_tick zero at tick zero passes, advance-ticks rejected during loading).
Cumulative gates: `cargo fmt --check` (clean), `cargo build --locked` (clean), `cargo clippy --locked -- -D warnings` (clean), `cargo test --locked` (315/315 pass)
...
NOTE (H2 review fix): per-command state application is a documented stub — `phase_apply_scheduled_commands` drains queued commands but applies no domain effects yet; the P1-12 claim is "commands are sequenced, queued, drained, and idempotently acknowledged".  Per-command state application lands in a later increment (P1-15+).

Independent review (codebase-review subagent) found 2 🔴 bugs: idempotency envelope comparison only checked expected_tick (not full command body), and expected_tick validation with no GameState returned wrong error code. Both fixed. Wildcard arm in `ReplayableGameCommand::from` replaced with `unreachable!()`. Redundant clone in scenario.rs determined to be necessary (ownership) and not changed. New test `same_id_different_command_body_conflict` verifies the hash-based idempotency fix.```
...
Scenarios activated: none (persistence/infrastructure-only increment)
Golden/hash changes: none
Notes: New module `engine/src/persistence.rs` (~1,000 lines). SaveEnvelope V1 with format_version, hash_scheme, schema_version, content_version, content_hash, state_hash, timestamp, normalized game_state. Synchronous atomic write (sync_atomic_write) using exclusive-create temp file, write+sync, rename, directory sync; Failure cleans up temp file, leaves prior save intact. Load validation checks format_version, hash_scheme, content version/hash, state hash, schema version, command-log invariants (unique IDs, strictly increasing server_sequence, implied-acceptance-tick nondecreasing, outcome-specific effective_tick rules). Rebuild pending commands from Accepted+ScheduledTick records, seed session receipts from full log. Migration framework: Migration trait + MigrationRegistry with contiguous chain validation. Actor integration: SaveNow calls sync_atomic_write directly (ADR-0008 §4 barrier), LoadAutosave reads/validates/rebuilds/publishes state. All 7 review bugs fixed: (1) SaveNow synchronous acknowledgment, (2) rebuild_pending_from_log filters application_boundary, (3) Rejected effective_tick <= tick+1, (4) next_session_sequence max() guard, (5) content hash error propagation, (6) implied-acceptance-tick invariant, (7) removed unused async worker channels.

NOTE (H3/H4 review fixes): replay coverage is now explicit — accepted replayable commands are written to `command_log` at acceptance (H2/H3), and a replay test (`running_queued_commands_survive_save_load`) queues commands during Running at ticks N and N+1, snapshots the game state via `SaveEnvelope::new`, and asserts exact state-hash + command-log equivalence and pending-schedule rebuild from the saved state (`rebuild_pending_from_log`). It constructs the envelope and rebuilds pending directly — it does not exercise the actor's `LoadAutosave` arm or the persistence load/validation path (those are covered by `save_load_round_trip` / `command_log_replay_equivalence` and later increments). SaveNow failures surface as `CommandFailed` with a `Failed` receipt (ADR-0008 §4 mandatory `Failed` semantics), not a silent success.```
```text
Increment: P1-14
Date: 2026-08-06
...
Focused proof: `cargo test --lib` — 340 tests including 7 API integration tests (status, state unavailable, content, auth required, collection unavailable, command newgame, 404 error envelope). Server spawns with real content catalog, actor, and router on random loopback port; tests exercise discovery-like status, Bearer auth with missing/bad/correct tokens, GameUnavailable before load, content catalog retrieval, collection query (503 before load), command envelope acceptance (NewGame completes synchronously to Paused with the canonical starting state), and 404 error envelope format.
Cumulative gates: `cargo fmt` (clean), `cargo build` (clean), `cargo clippy --no-deps` (clean), `cargo test --lib` (340/340 pass), `scripts/schema-export.sh` (37 schemas)
Scenarios activated: none (infrastructure-only increment — API server and generated client, no tick-level scenario)
Golden/hash changes: none
Notes: New module `engine/src/api.rs` (~410 lines) — Axum REST API server with auth middleware, error envelope, CORS loopback-only, 1 MiB request body limit. New binary `engine/src/main.rs` (~290 lines) — CLI with clap (content-dir, save-dir, port, insecure-no-auth), loopback port binding with 10-try fallback, random base64url token via getrandom, discovery file (connection.json with 0o600 permissions), content loading from JSON files, autoload NewGame at startup. 7 integration tests spawn real actor + Axum server on random port and verify all endpoints. Python client `python/steel_horizons_client/__init__.py` (~120 lines) — SteelHorizonsClient class with from_discovery factory, methods for status/state/content/collection/entity/command. Schema export extended with ServerStatus and LoadingStatus schemas (37 total). Lifecycle types gained JsonSchema derives and Serialize/Deserialize for API serialization. Dependencies added: tower-http (cors, limit features), serde (rc feature for Arc serialization).

NOTE (H1/M4/M6/L2/L4/L7 review fixes): NewGame and LoadAutosave command acceptance now routes synchronously to `handle_load_game`, completing Loading -> Paused/Won in-process instead of leaving the actor stuck in Loading (H1). Command submission unwraps in `api.rs` were replaced with typed error envelopes (M4); collection single-item lookup now also serves `research` and `build-orders` so listing and lookup agree (M6); API tests poll `/api/v1/status` readiness instead of fixed sleeps (L2) and `test_command_newgame` asserts the body unconditionally (L4); CORS was tightened to exact loopback hosts only, dropping the `null` origin (L7).

NOTE (deferred, L6): the walking-skeleton tick driver advances via a forced/manual scheduler-tick-advance (the actor's `handle_scheduler_tick`/`handle_advance_ticks` calling `tick.rs` `advance_one_tick`). A real tick-scheduler loop with autonomous time-driven advancement is deferred to a later increment; do not mistake the current manual advance for the final scheduler design.```

```text
Increment: review-fix batch (F1-F10), docs/REVIEW_FIX_PLAN.md
Date: 2026-08-08
Commit: 50a1618
Requirements: docs/REVIEW_FIX_PLAN.md — senior-engineering review findings F1-F10 across P1-01..P1-14 scope
Focused proof: `cargo test --locked` — 343 tests (was 340) including 3 new wire API tests: `test_command_advance_ticks`, `test_command_advance_ticks_rejected_when_running`, `test_command_newgame_unknown_scenario_rejected`. Client/package smoke tests: `python/tests/test_client_smoke.py` and `text-ui/tests/test_package_smoke.py` (deterministic, network-free; mocked discovery file + httpx transport).
Cumulative gates: `cargo fmt --check` (clean), `cargo clippy --locked --all-targets --all-features -- -D warnings` (clean), `cargo test --locked` (343/343 pass), `scripts/schema-export.sh --check` (37 schemas unchanged), `scripts/check-protocol-sync.py` (PASS), text-ui `ruff check src/ tests/` + `mypy src/` + `pytest` (2 pass), python `ruff check .` + `mypy steel_horizons_client/` + `pytest` (2 pass)
Scenarios activated: none (review-fix/quality increment)
Golden/hash changes: none
Notes: F1 — wire `Command::AdvanceTicks` now routes through `handle_advance_ticks` (was a broken stub that stuck the actor in Advancing); ack folds `AdvanceTicksCompleted{resulting_tick}`; snapshot published AFTER returning to Paused so served state reflects Paused. F2 — `check.sh` now gates `python/` (ruff+mypy+pytest) and the generated client meets the repo lint standard (PEP 585/604 modernization, `cast` for json); client smoke test added. F3 — typed error enums `FingerprintError`, `ReceiptMergeError`, `MigrationError`, `ContentHashError` replace `Result<_, String>` across command/persistence/actor (no `Result<_, String>` remains crate-wide). F4 — LoadAutosave propagates content-hash failure (restore prior state, typed load error) instead of `"unknown"` dummy fallback, matching SaveNow's no-dummy-hash policy. F5 — crate-level `deny(missing_docs)` removed (it was silently defeated by module-level allows). F6 — `check.sh` pytest swallow removed; both packages now have real pytest tests and `[tool.pytest.ini_options]` config. F7 — evidence-log entries reordered so P1-01 precedes P1-02a/P1-02b. F8 — stale `handle_load_game` doc comment updated to the real load/persist flow. F9 — NewGame wire `scenario_id` validated against `content.starting_system.id`, unknown ids rejected with `UnknownScenario`; wire test uses `"starting_system"`. F10 — `run()` single-branch `tokio::select!` simplified to `while let` recv loop. Reviewer (codebase-review subagent) confirmed all 10 mandated fixes + mandated test assertions present, 343 tests, 37 schemas unchanged. Fix plan `docs/REVIEW_FIX_PLAN.md` removed after review.```